  ...
  const handleQuery = useCallback(
    async (q: string) => {
      ...
      try {
        ...
        if (
          lower.includes("sprint health") ||
          lower.includes("sprint progress")
        ) {
          navigate("/spaces");
          onClose();
          return;
        ...
        if (
          lower.includes("blocked tickets") &&
          lower.includes("next sprint")
        ) {
          setResult({
            type: "action",
            summary:
              "EOS identified 3 blocked tickets in the current sprint. Moving them to next sprint will free up 8 story points.",
            items: [
              {
                label: "TRK-142",
                value: "Auth token refresh failing on mobile",
                meta: "5 pts · Blocked 48h",
              },
              {
                label: "TRK-156",
                value: "Payment webhook timeout",
                meta: "2 pts · Blocked 12h",
              },
              {
                label: "TRK-163",
                value: "CSV export memory leak",
                meta: "1 pt · Blocked 6h",
              },
            ],
        ...
        if (lower.includes("hasn't logged") || lower.includes("not logged")) {
          setResult({
            type: "list",
            summary:
              "EOS checked time logs for this week. 2 team members have not logged any work.",
            items: [
              {
                label: "Arjun M.",
                value: "0 hours logged this week",
                meta: "Last logged: 3 days ago",
              },
              {
                label: "Sara K.",
                value: "0 hours logged this week",
                meta: "Last logged: 5 days ago",
              },
        ...
        if (lower.includes("high priority") || lower.includes("critical bug")) {
          setResult({
            type: "list",
            summary:
              "EOS found 4 high-priority or critical bugs currently open.",
            items: [
              {
                label: "TRK-142",
                value: "Auth token refresh failing on mobile",
              ...
              {
                label: "TRK-151",
                value: "Payment form layout breaks on Safari",
                meta: "High · Unassigned",
              },
              {
                label: "TRK-159",
                value: "Dashboard charts not loading for EU users",
                meta: "High · Rahul",
              },
              {
                label: "TRK-164",
                value: "Export produces corrupt CSV",
                meta: "High · Arjun",
              },
            ],
        ...
        setResult({
          type: "answer",
          summary: "EOS could not process that command right now.",
        });
        setDone(true);
      } finally {
        setLoading(false);
    ...
    <div className={styles.overlay} onClick={onClose}>
      ...
      >
        ...
        <div className={styles.footer}>
          ...
          <span className={styles.footerEos}>
            <RiSparklingLine size={10} /> Powered by EOS
          </span>
        </div>
      </motion.div>
    </div>
