...
function processMeetingNotes(text: string): string {
  ...
  const decisions = lines
    .filter((l) => /decided|agreed|confirmed|approved/i.test(l))
    .slice(0, 3);
  const actions = lines
    .filter((l) => /will|todo|action item|follow.?up/i.test(l))
    .slice(0, 4);
...
function KnowledgeMapPanel({
  ...
  const nodes = shown.map((p, i) => {
    ...
    if (isActive)
      return { ...p, x: cx, y: cy, isActive: true, color: "var(--accent)" };
  ...
  return (
    <div className={styles.mapWrap}>
        ...
        {nodes.map((n) => (
          <g key={n.id}>
            <circle
              ...
              r={n.isActive ? 10 : 6}
              fill={n.color}
              fillOpacity={n.isActive ? 0.22 : 0.15}
              stroke={n.color}
              strokeWidth={n.isActive ? 2 : 1.2}
            />
            {n.isActive && (
              <circle
                cx={n.x}
            ...
            <text
              x={n.x}
              y={n.y + (n.isActive ? 24 : 17)}
              textAnchor="middle"
              fontSize={n.isActive ? "8" : "7"}
              fill="var(--text-3)"
              fontFamily="inherit"
            >
              {n.title.slice(0, 14)}
      ...
      {shown.length === 0 && (
        <p className={styles.mapEmpty}>Add pages to see their connections</p>
      )}
    </div>
  );
}
...
export default function WikiPage() {
  ...
  // Reset selection on mount so nothing is pre-selected when landing fresh
  useEffect(() => {
    if (!urlPageId) {
      setActivePage(null);
      setActiveSpace(null);
  ...
  const createPageMut = useMutation({
    mutationFn: (p: { title: string; content: string; parent_id?: string }) =>
      createWikiPage({ space_id: activeSpaceId!, ...p }),
    onSuccess: (p) => {
    ...
    try {
      ...
      const structured =
        typeof result === "string"
          ? result
          : (result?.structured_md ??
            result?.content ??
            processMeetingNotes(captureInput));
  ...
  const spaceHealthById = new Map(
    (intelligence?.spaces ?? []).map((s) => [s.space_id, s]),
  );
    ...
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div className={styles.pageHeaderLeft}>
          <h1 className={styles.pageTitle}>Wiki</h1>
        </div>
      </div>
      ...
      <div className={styles.pageSubHeader}>
        ...
        <div className={styles.pageHeaderRight}>
          <button
            ...
            title="Structure meeting notes into a doc"
          >
          ...
          {activePage && (
            <button
              className={`${styles.aiPanelBtn} ${showAIPanel ? styles.aiPanelBtnActive : ""}`}
              onClick={() => setShowAIPanel((v) => !v)}
              title="Toggle AI panel"
            >
              <RiLayoutRightLine size={13} />
              AI Panel
            </button>
          )}
          {activeSpaceId && (
      ...
      <div className={styles.body}>
        ...
        <aside className={styles.sidebar}>
          <div className={styles.sidebarScroll}>
            ...
            <div className={styles.sideSection}>
              <div className={styles.sideSectionHeader}>
                <span>Spaces</span>
                <button
                  className={styles.sideSectionAdd}
                      ...
                      <div className={styles.spaceRing}>
                        <svg width="20" height="20" viewBox="0 0 20 20">
                          ...
                          <circle
                            ...
                            strokeWidth="2.5"
                            strokeLinecap="round"
                            strokeDasharray={`${(h / 100) * circ} ${circ}`}
                            transform="rotate(-90 10 10)"
                          />
                        </svg>
                        <span
                          className={styles.spaceRingNum}
                          style={{ color: c }}
                        >
                          {h}
                        </span>
          ...
          {/* EOS Quick Actions — pinned to sidebar bottom */}
          <div className={styles.eosActions}>
            <div className={styles.eosActionsLabel}>
            ...
            {[
              {
                ...
                label: "Scan stale pages",
                cmd: "Find stale pages in this wiki space",
              },
              {
                icon: <RiAlertLine size={11} />,
                label: "Detect conflicts",
                cmd: "Detect documentation conflicts for the active page",
              },
              {
                icon: <RiTeamLine size={11} />,
                label: "Generate onboarding path",
                onboard: true,
              },
              {
                icon: <RiLightbulbLine size={11} />,
                label: "Find coverage gaps",
                cmd: "Find documentation coverage gaps in this wiki space",
              },
            ].map((a, i) => (
              <button
                key={i}
                className={styles.eosActionItem}
                onClick={() => {
                  if ("onboard" in a) {
          ...
          {activePage ? (
            <>
              <div className={styles.mainTopbar}>
                <div className={styles.breadcrumb}>
                  ...
                  />
                  <span className={styles.breadcrumbSpace}>
                    {activeSpace?.name}
                  </span>
                  {breadcrumbs.map((b) => (
                ...
                <div className={styles.pageActions}>
                  ...
                  >
                    <MdHistory size={13} style={{ marginRight: 4 }} />
                    History
                  </button>
                  <button
                    className="btn btn-ghost btn-sm"
                  ...
                  >
                    <MdDeleteOutline size={13} style={{ marginRight: 4 }} />
                    Delete
                  </button>
                </div>
              </div>
              ...
              {pageInsights && (
                <div
                  className={`${styles.eosBanner} ${styles[`eosBanner_${pageInsights.freshness}`]}`}
                >
                  <span
                    className={styles.eosBannerDot}
                    style={{ background: FRESH_COLOR[pageInsights.freshness] }}
                  />
                  ...
                  <span
                    className={styles.eosBannerBadge}
                    style={{
                      color: FRESH_COLOR[pageInsights.freshness],
                      borderColor: FRESH_COLOR[pageInsights.freshness],
                    }}
                  >
                    {pageInsights.freshness === "fresh"
                      ? "Fresh"
                      : pageInsights.freshness === "aging"
                        ? "Aging"
                        : "Needs update"}
                  </span>
                </div>
              ...
              {showVersions && (
                <div className={styles.versionsPanel}>
                  <div className={styles.versionsPanelTitle}>
                    <RiGitMergeLine size={11} />
                    Smart History · semantic diff enabled
                  </div>
                  {versions.length === 0 && (
                    <p className={styles.emptyTree}>
            ...
            <div className={styles.emptyState}>
              ...
              <h2 className={styles.emptyTitle}>
                {activeSpaceId ? "Select a page" : "Choose a space"}
              </h2>
              <p className={styles.emptyDesc}>
                {activeSpaceId
                  ? "Pick a page from the sidebar or create something new."
                  : "Select a space from the sidebar to start writing."}
              </p>
                ...
                <>
                  ...
                  <div className={styles.emptyFeatures}>
                    <div className={styles.emptyFeatureItem}>
                      ...
                      <span>
                        EOS auto-scans every page for freshness and conflicts
                      </span>
                    </div>
                    <div className={styles.emptyFeatureItem}>
                      <RiNodeTree size={12} />
                      <span>Knowledge map shows how your pages connect</span>
                    </div>
                    <div className={styles.emptyFeatureItem}>
                      <RiGitMergeLine size={12} />
                      <span>
                        Smart history with semantic diff across versions
                      </span>
                    </div>
                    <div className={styles.emptyFeatureItem}>
                      <RiFileTextLine size={12} />
                      <span>Structure meeting notes into clean wiki pages</span>
                    </div>
                  </div>
                </>
              )}
              ...
              <div className={styles.emptyStats}>
                <div className={styles.emptyStat}>
                  <span className={styles.emptyStatVal}>{pages.length}</span>
                  <span className={styles.emptyStatLbl}>pages</span>
                </div>
                <div className={styles.emptyStat}>
                  <span className={styles.emptyStatVal}>{spaces.length}</span>
                  <span className={styles.emptyStatLbl}>spaces</span>
                </div>
              </div>
            </div>
          )}
      ...
      {activePage && pageInsights && (
        <SideDrawer
          ...
          stats={[
            {
              label: "Health",
              value: `${pageInsights.score}%`,
              color:
                pageInsights.score >= 75
                  ? "var(--green)"
                  : pageInsights.score >= 50
                    ? "var(--amber)"
                    : "var(--red)",
            },
          ...
          <div className={styles.aiTabs}>
            {(
              [
                ...
                {
                  id: "insights",
                  icon: <RiLightbulbLine size={12} />,
                  label: "Insights",
                },
                {
                  id: "chat",
                  icon: <RiBrainLine size={12} />,
                  label: "Ask EOS",
                },
              ] as { id: AITab; icon: React.ReactNode; label: string }[]
            ).map((t) => (
              <button
                key={t.id}
                className={`${styles.aiTab} ${aiTab === t.id ? styles.aiTabActive : ""}`}
          ...
          {aiTab === "health" && (
            <div className={styles.aiTabContent}>
                ...
                <div className={styles.healthMeta}>
                  ...
                  {[
                    ...
                    },
                    {
                      label: "Coverage",
                      val: Math.round(
                        (pageInsights.coverage.headings +
                          pageInsights.coverage.examples +
                          pageInsights.coverage.links +
                          pageInsights.coverage.diagrams) /
                          4,
                      ),
                    },
                    {
                      label: "Cross-links",
                      val: Math.min(100, 20 + pageInsights.related_count * 16),
                    },
                  ].map((m) => (
                    <div key={m.label} className={styles.healthMetaRow}>
                      ...
                      <div className={styles.healthMetaTrack}>
                        <div
                          className={styles.healthMetaFill}
                          style={{
                            width: `${m.val}%`,
                            background:
                              m.val >= 70
              ...
              <div className={styles.coverageSection}>
                <div className={styles.coverageSectionLabel}>
                  Content Coverage
                </div>
                  ...
                  {(
                    ...
                    <div key={l} className={styles.coverageItem}>
                      <div
                        ...
                        style={{
                          background: `conic-gradient(${v >= 75 ? "var(--green)" : v >= 50 ? "var(--amber)" : "var(--red)"} ${v}%, var(--surface-3) 0)`,
                        }}
                      />
          ...
          {aiTab === "map" && (
            <div className={styles.aiTabContent}>
              <div className={styles.mapHeader}>
                ...
                <span className={styles.mapHeaderSub}>
                  {pages.length} pages · {spaces.length} spaces
                </span>
              </div>
              <KnowledgeMapPanel pages={pages} activeId={activePageId} />
              <div className={styles.mapStats}>
                <div className={styles.mapStatItem}>
                  <span className={styles.mapStatVal}>
                    {intelligence?.map_stats.fresh ?? 0}
                  </span>
                  <span className={styles.mapStatLbl}>fresh</span>
                </div>
                <div className={styles.mapStatItem}>
                  <span
                    className={styles.mapStatVal}
                    style={{ color: "var(--amber)" }}
                  >
                    {intelligence?.map_stats.aging ?? 0}
                  </span>
                ...
                <div className={styles.mapStatItem}>
                  <span
                    ...
                    style={{ color: "var(--red)" }}
                  >
                    {intelligence?.map_stats.stale ?? 0}
                  </span>
            ...
            <div className={styles.aiTabContent}>
              <div className={styles.insightsList}>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className={styles.insightDesc}>
                        {pageInsights.days_old}d old ·{" "}
                        {pageInsights.linked_tickets} linked tickets found
                      </div>
                    </div>
                  </div>
                )}
                {pageInsights.has_conflict && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className={styles.insightDesc}>
                        Another page has conflicting information — review
                        recommended
                      </div>
                    </div>
                  </div>
                ...
                >
                  ...
                  <div>
                    <div className={styles.insightTitle}>
                      {pageInsights.related_count} related pages discovered
                    </div>
                    <div className={styles.insightDesc}>
                      Related pages can be linked to improve discoverability
                    </div>
                  </div>
                </div>
                <div
                  ...
                  <div>
                    <div className={styles.insightTitle}>
                      {pageInsights.compliance.passed
                        ...
                        : "Review sensitive strings"}
                    </div>
                    <div className={styles.insightDesc}>
                      {pageInsights.compliance.passed
                        ? "No sensitive data patterns detected"
                        : "Potential secrets or tokens may be present in this page"}
                    </div>
                  </div>
                </div>
              </div>
              ...
              {activePageId && (
                <RelatedDocsWidget
                  ...
                  onTicketSelect={(key) =>
                    navigate(`/backlog?search=${encodeURIComponent(key)}`)
                  }
                />
              )}
            </div>
            ...
            <div className={`${styles.aiTabContent} ${styles.chatTabContent}`}>
              {aiHistory.length === 0 && !aiThinking && (
                <div className={styles.chatSuggestions}>
                  <div className={styles.chatSuggestionsLabel}>
                    Quick actions
                  </div>
                  {[
                    "Summarize this page",
              ...
              <div className={styles.chatInputRow}>
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && void submitAiChat(aiInput)
                  }
                />
                <button
                  className={styles.chatSendBtn}
                  onClick={() => void submitAiChat(aiInput)}
      ...
      {showCapture && (
        <div
          className={styles.modalOverlay}
          onClick={(e) => e.target === e.currentTarget && setShowCapture(false)}
        >
            ...
            <div className={styles.modalHeader}>
              <div>
                ...
                <p className={styles.modalSub}>
                  Paste raw notes or a transcript — EOS will turn them into a
                  clean wiki draft
                </p>
              </div>
              <button
            ...
            {!captureOutput ? (
              <>
                <textarea
                  ...
                  placeholder="Paste meeting notes, bullet points, or a full transcript…"
                  value={captureInput}
                ...
                <div className={styles.captureActions}>
                  <button
                    className={styles.captureBtn2}
                    onClick={handleCapture}
                    disabled={!captureInput.trim() || captureProcessing}
                  >
                    {captureProcessing ? (
                      <>
                        <span className={styles.thinkDot} />
              ...
              <>
                <div className={styles.captureOutput}>
                  <div className={styles.captureOutputLabel}>
                    <RiSparklingLine size={11} />
                    EOS structured this for you
                  </div>
                ...
                <div className={styles.captureActions}>
                  ...
                  <button
                    className={styles.captureBtnSecondary}
                    onClick={() => setCaptureOutput(null)}
                  >
                    Edit notes
                  </button>
                </div>
      ...
      {showOnboarding && (
        <div
          className={styles.modalOverlay}
          onClick={(e) =>
            e.target === e.currentTarget && setShowOnboarding(false)
          }
        >
          <div className={`${styles.modal} ${styles.onboardingModal}`}>
            <div className={styles.modalHeader}>
              <div>
                ...
                <p className={styles.modalSub}>
                  EOS recommended reading order for new team members
                </p>
              </div>
              <button
                className={styles.modalClose}
            ...
            {pages.length === 0 ? (
              <p className={styles.emptyTree} style={{ margin: "16px 0" }}>
                Add pages to generate an onboarding path.
              </p>
            ) : (
                    ...
                    <div key={item.page_id} className={styles.onboardingItem}>
                      ...
                      <div className={styles.onboardingContent}>
                        ...
                        <div className={styles.onboardingItemMeta}>
                          <span className={styles.onboardingTag}>
                            {item.tag}
                          </span>
                      ...
                      <div
                        className={styles.onboardingScore}
                        style={{
                          color:
                            item.score >= 70
                              ? "var(--green)"
            ...
            <div className={styles.onboardingFooter}>
              <RiSparklingLine size={11} />
              <span>
                Path generated by EOS based on page structure and freshness
              </span>
            </div>
          </div>
        </div>
    ...
    <div>
      ...
      >
        ...
        <span
          ...
          style={{ background: FRESH_COLOR[freshness] }}
        />
