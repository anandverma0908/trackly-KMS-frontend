...
function parseRetroSections(text: string): RetroSection[] {
  ...
  const HEADERS: { pattern: RegExp; title: string }[] = [
    {
      pattern: /went well|positives?|strengths?|wins?|highlights?/i,
      title: "What went well",
    },
    {
      pattern: /didn.?t|negatives?|challenges?|delta|not well/i,
      title: "What didn't go well",
    },
    {
      pattern: /action items?|next steps?|actions?|todo/i,
      title: "Action items",
    },
  ];
...
function parseReleaseNoteSections(
  ...
  const TYPES: { pattern: RegExp; label: string }[] = [
    ...
    {
      pattern: /improvements?|chores?|maintenance|tasks?/i,
      label: "Improvements",
    },
  ];
  for (const line of lines) {
...
function ConfBar({ value }: { value: number }) {
  ...
  return (
    <div className={styles.confRow}>
      ...
      <span className={styles.confValue} style={{ color }}>
        {value}%
      </span>
    </div>
  );
...
function ReleaseContent({
  ...
  data: { version: string; sections: { type: string; items: string[] }[] };
}) {
    ...
    <div className={styles.drawerContent}>
      <div className={styles.debtGaugeRow}>
        <div className={styles.debtGauge}>
          <div
            className={styles.debtGaugeFill}
            style={{
          ...
          />
          <div
            className={styles.debtThresholdLine}
            style={{ left: `${(data.threshold / 40) * 100}%` }}
          />
        </div>
        <span className={styles.debtPct} style={{ color: overallColor }}>
          {pct}%
        </span>
      </div>
      <div className={styles.debtMeta}>
        <span className={styles.debtLabel} style={{ color: overallColor }}>
          {over
            ? `${pct - data.threshold}pp above safe threshold`
            : "Within safe threshold"}
        </span>
        <span className={styles.debtThreshLabel}>
          Safe limit: {data.threshold}%
        </span>
      </div>
        ...
        <div key={m.name} className={styles.memberCard}>
          ...
          <div className={styles.memberCardBody}>
            <div className={styles.memberNameRow}>
              ...
              {m.overloaded && (
                <span className={styles.overloadBadge}>Overloaded</span>
              )}
            </div>
            ...
            <div className={styles.utilRow}>
              ...
              <span
                className={styles.utilPct}
                style={{ color: m.overloaded ? "var(--red)" : "var(--text-3)" }}
              >
                {m.utilization}%
              </span>
            </div>
...
function ForecastContent({ data }: { data: SprintForecast }) {
  ...
  const color =
    prob >= 80 ? "var(--green)" : prob >= 60 ? "var(--amber)" : "var(--red)";
  const r = 26;
        ...
        <div className={styles.forecastRing}>
          <svg width={72} height={72} style={{ transform: "rotate(-90deg)" }}>
            ...
            <circle
              ...
              strokeDashoffset={circ * (1 - prob / 100)}
              strokeLinecap="round"
            />
          </svg>
          <span className={styles.forecastPct} style={{ color }}>
            {prob}%
          </span>
        </div>
        <div className={styles.forecastMeta}>
          ...
          {data.predicted_completion_date && (
            <p className={styles.forecastNote}>
              Predicted: {data.predicted_completion_date}
            </p>
          )}
          <p className={styles.forecastNote}>
            Historical accuracy: {Math.round(data.historical_accuracy)}%
          </p>
          <p className={styles.forecastNote}>
            Range: {Math.round(data.confidence_interval.lower)}% –{" "}
            {Math.round(data.confidence_interval.upper)}%
          </p>
        </div>
      </div>
      {data.nova_summary && (
        <p className={styles.bodyText}>{data.nova_summary}</p>
      )}
    ...
    <div className={styles.drawerContent}>
      <div className={styles.forecastProbRow}>
        ...
        <div className={styles.forecastMeta}>
          ...
          <p className={styles.forecastNote} style={{ color }}>
            {label}
          </p>
        </div>
      </div>
...
function SilentBlockersContent({
  data,
}: {
  data: {
    blockers: {
  ...
  if (!data.blockers?.length) {
    return (
      <div className={styles.drawerContent}>
        <div className={styles.emptyDrawer}>
          <RiEyeLine size={28} color="var(--text-3)" />
          <p>No silent blockers detected — ticket flow looks healthy.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {drawer.loading ? (
        ...
        <div className={styles.emptyDrawer}>
          <p>No data returned. Try again.</p>
        </div>
      ) : drawer.type === "retro" ? (
        ...
        <AnomalyContent
          data={drawer.data.anomalies}
          summary={drawer.data.summary}
        />
      ) : drawer.type === "gaps" ? (
        <GapsContent data={drawer.data} />
      ) : drawer.type === "sprint_fail" ? (
        <SprintFailContent data={drawer.data} />
      ) : drawer.type === "silent_blockers" ? (
        <SilentBlockersContent data={drawer.data} />
      ) : null}
    </SideDrawer>
  );
}
...
Answer concisely and helpfully, referencing sprint data where relevant.`,
  ...
  function resolve(data: any) {
    setDrawer((p) => (p ? { ...p, loading: false, data } : null));
  }
  ...
  function sprintContext() {
    if (!activeSprint)
      return `Project: ${project.name} (pod: ${pod}), no active sprint.`;
...
Team (${project.members.length}): ${project.members.map((m) => `${m.name} (${m.role})`).join(", ")}.`;
  ...
  async function handleRetro() {
    ...
    open("retro", "Sprint Retrospective");
    try {
      const ctx = sprintContext();
      const res = await novaQuery(
...
[{"title": string, "description": string, "impact": "high"|"medium"|"low", "confidence": 0-100, "mitigation": string}]`,
  ...
  async function handleDebt() {
    ...
    open("debt", "Technical Debt Radar");
    try {
      const ctx = sprintContext();
      const bugCount =
        activeSprint?.tasks.filter((t) => t.type === "Bug").length ?? 0;
...
score = how severe the debt is (0=none, 100=critical). Be data-driven: code debt from bug ratio, test debt from bug recurrence, doc debt from knowledge gap signals, dependency debt from blocked tickets.`,
      ...
      const fallbackPct = Math.min(
        40,
        Math.round((bugCount / total) * 100) + 8,
      );
      resolve({
        dimensions: parsed?.dimensions ?? [
          {
            name: "Code Debt",
            score: fallbackPct + 5,
            detail: `${bugCount} bugs out of ${total} tickets indicates accumulated code quality issues.`,
          },
          {
            name: "Test Debt",
            score: Math.max(10, fallbackPct - 5),
            detail: "Recurring bugs suggest insufficient test coverage.",
          },
          {
            name: "Documentation Debt",
            score: 35,
            detail:
              "Ticket patterns show undocumented areas causing repeated issues.",
          },
          {
            name: "Dependency Debt",
            score: 20,
            detail:
              "Blocked tickets may indicate outdated or fragile dependencies.",
          },
        ],
        ...
        recommendation:
          ...
          "Allocate 20% of next sprint to debt reduction, prioritising code and test debt.",
      });
    } catch (e: any) {
  ...
  async function handleTeamPerf() {
    ...
    open("teamperf", "Team Performance");
    try {
      // Derive per-member stats purely from task data
      const memberStats = project.members
        .map((member) => {
          const assigned = activeSprint.tasks.filter(
            (t) => t.assignee === member.name,
          );
      ...
      const avgPoints =
        memberStats.length > 0
          ? memberStats.reduce((a, m) => a + m.points, 0) / memberStats.length
          : 1;
      ...
      const members = memberStats.map((m) => ({
        ...
        utilization: Math.round(
          (m.points / Math.max(avgPoints * 1.5, 1)) * 100,
        ),
...
In one sentence, identify the most important workload pattern or imbalance to address.`,
  ...
  async function handleClient() {
    ...
    open("client", "Client Status Update");
    try {
      const ctx = sprintContext();
      const res = await novaQuery(
...
Tone: professional, positive, concise. No bullet points — flowing prose only.`,
  ...
  async function handleForecast() {
    ...
    open("forecast", "Sprint Health Forecast");
    try {
      const ctx = sprintContext();
      const pct =
        activeSprint.totalPoints > 0
...
current_probability = likelihood (0-100) the sprint completes on time. Be data-driven.`,
      ...
      resolve({
        ...
        confidence_interval: {
          lower: Math.max(0, (parsed?.current_probability ?? pct) - 15),
          upper: Math.min(100, (parsed?.current_probability ?? pct) + 15),
        },
  ...
  async function handleAnomaly() {
    ...
    open("anomaly", "Anomaly Detection");
    try {
      const ctx = sprintContext();
      const today = new Date().toISOString().slice(0, 10);
      const res = await novaQuery(
...
If everything looks normal return an empty anomalies array with a positive nova_summary.`,
      ...
      resolve({
        ...
        summary: parsed?.nova_summary ?? res.answer,
      });
    } catch (e: any) {
  ...
  async function handleSprintFail() {
    if (!activeSprint) return;
    setActiveLoading("sprint_fail");
    open("sprint_fail", "Sprint Failure Predictor");
    try {
      const ctx = sprintContext();
      const res = await novaQuery(
...
probability = likelihood the sprint will NOT complete. Be data-driven: consider blocked tickets, bug ratio, WIP, team capacity, and progress vs timeline.`,
      ...
      const blocked = activeSprint.tasks.filter(
        (t) => t.status === "Blocked",
      ).length;
      const fallbackProb = Math.min(
        90,
        blocked * 20 + (bugs > 3 ? 15 : 0) + Math.max(0, 60 - pct),
      );
      resolve({
        ...
        warning_signs:
          ...
          (blocked > 0
            ? [
                {
                  sign: `${blocked} blocked ticket(s)`,
                  severity: "high",
                  detail: "Blockers directly impede sprint completion.",
                },
              ]
            : []),
        root_causes: parsed?.root_causes ?? [
          "Sprint data insufficient for full analysis",
        ],
        recommendation:
          parsed?.recommendation ??
          "Review blocked tickets immediately and escalate unresolved dependencies.",
      });
    } catch (e: any) {
      toast.error(e?.message ?? "Failure prediction failed");
      closeDrawer();
    } finally {
      setActiveLoading(null);
    }
  ...
  async function handleSilentBlockers() {
    if (!activeSprint) return;
    setActiveLoading("silent_blockers");
    open("silent_blockers", "Silent Blocker Detection");
    try {
      const ctx = sprintContext();
      const res = await novaQuery(
...
Focus on In Progress or To Do tickets that haven't moved. Return empty array if none found.`,
      ...
      resolve({
        ...
        summary: parsed?.summary ?? res.answer,
      });
    } catch (e: any) {
      toast.error(e?.message ?? "Silent blocker scan failed");
      closeDrawer();
    } finally {
      setActiveLoading(null);
    }
  ...
  async function handleGaps() {
    ...
    open("gaps", "Knowledge Gap Analysis");
    try {
      const ctx = sprintContext();
      const res = await novaQuery(
...
[{"id": string, "topic": string, "suggestion": string, "ticket_count": number, "wiki_coverage": 0-100, "example_tickets": [string], "detected_at": null}]`,
  ...
  const ALL_CARDS: {
  ...
  }[] = [
    {
      ...
      description:
        "Structured retro from Done tickets — What went well, What didn't, Action items.",
      cta: "Generate Retro",
      handler: handleRetro,
      requiresSprint: true,
    },
    {
      ...
      title: "Release Notes",
      description:
        "Changelog grouped by Features, Bug Fixes, and Improvements — ready to share.",
      cta: "Generate Notes",
    ...
    },
    {
      ...
      description:
        "Top risks with impact rating, confidence level, and EOS mitigation strategies.",
      cta: "Analyse Risks",
      handler: handleRisk,
    ...
    },
    {
      ...
      description:
        "Multi-dimensional debt scan: code, test, docs, and dependency debt — with severity scores.",
      cta: "Run Debt Radar",
      handler: handleDebt,
      requiresSprint: false,
    },
    {
      ...
      description:
        "Workload distribution and utilisation across team members in the active sprint.",
      cta: "Analyse Team",
    ...
    },
    {
      ...
      description:
        "A polished, professional project update — written by EOS and ready to send.",
      cta: "Generate Update",
    ...
    },
    {
      ...
      description:
        "Completion probability, risk factors, and suggestions for the active sprint.",
      cta: "Forecast Sprint",
    ...
    },
    {
      ...
      description:
        "Statistical drift in velocity, bug rates, and PR review times — caught early.",
      cta: "Run Scan",
    ...
    },
    {
      ...
      description:
        "Wiki coverage gaps from ticket topics, with EOS training recommendations.",
      cta: "Identify Gaps",
      handler: handleGaps,
      requiresSprint: false,
    },
    {
      ...
      description:
        "Probability the sprint will not complete — based on blockers, bugs, WIP, and pace.",
    ...
    },
    {
      ...
      description:
        "Tickets not marked Blocked but showing hidden dependency or stall signals.",
