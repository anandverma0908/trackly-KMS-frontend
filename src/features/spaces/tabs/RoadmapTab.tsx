    ...
    <div className={styles.tab}>
      ...
      <div className={styles.kpiRow}>
        {[
          ...
          {
            label: "Epics",
            value: String(project.epics.length),
            sub: `${atRiskEpics.length} at risk`,
            icon: <RiFlag2Line />,
            color: "var(--purple, #A78BFA)",
          },
          {
            label: "Sprints",
            value: String(project.sprints.length),
            sub: `${project.sprints.filter((s) => s.status === "completed").length} completed`,
            icon: <RiLoopLeftLine />,
            color: "var(--green)",
          },
          {
          ...
          <div
            ...
            style={{ "--kc": card.color } as React.CSSProperties}
          >
        ...
        <div className={styles.briefBar}>
          ...
          <div className={styles.briefContent}>
            ...
            <div className={styles.briefText}>
              ...
              {narrative.data && (
                <span>{narrative.data.answer.split("\n")[0]}</span>
              )}
              {!narrative.isPending &&
                !narrative.isError &&
                !narrative.data && (
      ...
      {/* ── Risk Alerts + Epic Breakdown (side by side) ── */}
      {project.epics.length > 0 && (
        <div className={styles.riskEpicRow}>
          {/* EOS Risk Signals card */}
          <div className={styles.riskCard}>
            <div className={styles.epicCardsTitle}> EOS Risk Signals </div>
            {atRiskEpics.length === 0 && overdueEpics.length === 0 ? (
              <div className={styles.riskEmpty}>
                <span style={{ color: "var(--green)" }}>✓</span> No timeline
                risks detected
              </div>
            ) : (
              <div className={styles.riskAlerts}>
                {overdueEpics.map((e) => (
                  ...
                  >
                    <span>
                      <strong>{e.title}</strong> is overdue ({e.progress}%
                ...
                {atRiskEpics.map((e) => (
                  ...
                  >
                    ...
                    <span>
                      <strong>{e.title}</strong> is behind schedule (
                      {e.progress}% complete).
                    </span>
                  </div>
                ))}
                  ...
                  >
                    <div className={styles.epicCardTop}>
                      <div className={styles.epicCardTitle}>
                        ...
                        {isOverdue && (
                          <span
                            className={styles.epicRiskBadge}
                            style={{
                              color: "var(--red)",
                              borderColor: "var(--red)",
                              background:
                                "var(--red-glow, rgba(248,113,113,0.1))",
                            }}
                          >
                            Overdue
                          </span>
                      ...
                      <div
                        className={styles.epicCardPct}
                        style={{ color: epic.color }}
                      >
                        {epic.progress}%
                      </div>
                    </div>
                    <div className={styles.epicProgressRow}>
                      <div className={styles.epicProgressBar}>
                        <LinearProgress
                          variant="determinate"
                          value={epic.progress}
                          sx={{
                          ...
                          <Tooltip
                            title={
                              <div>
                                ...
                                <br />
                                {epic.completed}/{epic.tasks} tasks ·{" "}
                                {epic.progress}% done
                              </div>
                            }
                            arrow
                  ...
                  {project.sprints.map((sprint) => {
                    ...
                    const pct =
                      sprint.totalPoints > 0
                        ? Math.round(
                            ((sprint.donePoints ?? 0) / sprint.totalPoints) *
                              100,
                          )
                        : 0;
                    return (
