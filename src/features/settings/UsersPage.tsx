...
export default function UsersPage() {
  ...
  function togglePod(pod: string) {
    setForm((f) => ({
      ...f,
      pods: f.pods.includes(pod)
        ? f.pods.filter((p) => p !== pod)
        : [...f.pods, pod],
    }));
  }
  ...
  const COLUMNS: Column<UserRow>[] = [
        ...
        <div className={styles.pods}>
          {u.pod ? (
            u.pod
              ...
              .filter(Boolean)
              .map((p) => (
                <span key={p} className={styles.podChip}>
                  {p}
                </span>
              ))
          ) : (
    ...
    },
    {
      ...
      render: (u) => (
        <span
          className={`${styles.status} ${u.status === "active" ? styles.active : styles.pending}`}
        >
          {u.status}
        </span>
      ),
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={`${styles.header} fade-up`}>
        <div>
          ...
          <p className={styles.subtitle}>
            {users.length} users in your organisation
          </p>
        </div>
        <button
      ...
      <LatticeGrid<UserRow>
        ...
        footerLeft={
          !isLoading ? `${filtered.length} of ${users.length} users` : undefined
        }
      />
            ...
            <div className={styles.modalBody}>
                  ...
                  <div className={styles.fieldGroup}>
                    <label className={styles.fieldLabel}>
                      ...
                      <span className={styles.optional}>
                        (optional — user can set later)
                      </span>
                    </label>
                    <input
                      className={styles.fieldInput}
                      placeholder="e.g. empCode"
                      type="password"
                      value={form.password}
                      onChange={(e) =>
              ...
              <div className={styles.fieldGroup}>
                ...
                <div className={styles.podGrid}>
                  {ALL_PODS.map((pod) => (
                    <button
                      key={pod}
                      type="button"
                      className={`${styles.podToggle} ${form.pods.includes(pod) ? styles.podToggleOn : ""}`}
                ...
                <div className={styles.podHint}>
                  {form.pods.length === 0
                    ? "No PODs selected"
                    : `${form.pods.length} selected: ${form.pods.join(", ")}`}
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div className={styles.modalBody}>
              <p className={styles.resetInfo}>
                Setting a new password for <strong>{resetModal.name}</strong>
              </p>
              <div className={styles.fieldGroup}>
            ...
            <div className={styles.modalFooter}>
              ...
              <button
                className="btn btn-primary btn-sm"
                disabled={newPwd.length < 6 || resetMutation.isPending}
                onClick={() =>
                  resetMutation.mutate({ id: resetModal.id, pwd: newPwd })
                }
              >
